...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return {
    title: l === "fr" ? "À propos — FireSight" : "About — FireSight",
    description:
      l === "fr"
        ? "Comment FireSight détecte les départs de feu de forêt : satellites NASA FIRMS et Meteosat MTG, veille citoyenne vérifiée par IA, limites et avertissements."
        : "How FireSight detects wildfire ignitions: NASA FIRMS and Meteosat MTG satellites, AI-verified citizen reports, limits and disclaimers.",
    alternates: {
...
function faqJsonLd(l: Lang) {
  const qa =
    l === "fr"
      ? [
          ["Qu'est-ce que FireSight ?", "FireSight est une carte mondiale gratuite des feux de forêt en temps quasi réel. Elle combine les détections satellite (NASA FIRMS/VIIRS, GOES, Meteosat MTG) et les témoignages citoyens vérifiés par IA pour repérer les départs de feu le plus tôt possible, souvent avant les médias."],
          ["D'où viennent les données de FireSight ?", "Des instruments satellite VIIRS (résolution 375 m, NASA FIRMS), des satellites géostationnaires GOES (Amériques) et Meteosat MTG (Europe/Afrique, rafraîchi toutes les 10 minutes), et d'une veille des réseaux sociaux (Bluesky, presse, Telegram) dont chaque signalement est jugé deux fois par IA avant affichage."],
          ["FireSight est-il gratuit ?", "Oui. FireSight est un projet à mission : la carte, les alertes par zone et toutes les fonctionnalités sont gratuites."],
          ["FireSight remplace-t-il les secours ?", "Non. FireSight est un service d'information indépendant, pas un canal d'alerte officiel. En cas d'urgence, appelez le 112 (Europe) ou le 18 (France)."],
        ]
      : [
          ["What is FireSight?", "FireSight is a free near real-time world map of wildfires. It combines satellite detections (NASA FIRMS/VIIRS, GOES, Meteosat MTG) with AI-verified citizen reports to spot fire starts as early as possible, often before the media."],
          ["Where does FireSight's data come from?", "From VIIRS satellite instruments (375 m resolution, NASA FIRMS), the GOES (Americas) and Meteosat MTG (Europe/Africa, refreshed every 10 minutes) geostationary satellites, and social media monitoring (Bluesky, press, Telegram) where every report is judged twice by AI before being displayed."],
          ["Is FireSight free?", "Yes. FireSight is a mission-driven project: the map, area alerts and all features are free."],
          ["Does FireSight replace emergency services?", "No. FireSight is an independent information service, not an official alert channel. In an emergency, call 911 (US & Canada), 112 (Europe) or your local emergency number."],
        ];
  return {
        ...
        {lang === "fr" ? (
          <>
            <h1 className="mb-6" style={{ fontSize: "var(--text-h2)", color: "var(--ink)" }}>
              À propos de FireSight
            </h1>
            <section className="space-y-4 text-[15px] leading-relaxed">
              <p>
                FireSight est un service d&apos;information indépendant qui cartographie en temps quasi
              ...
              <p>
                ...
                précocité. FireSight y ajoute une veille des témoignages citoyens sur les réseaux
              ...
              <p className="rounded-[14px] p-4" style={{ background: "var(--ember-soft)", color: "#8C3A16" }}>
                FireSight n&apos;est pas un service d&apos;alerte officiel et ne remplace en aucun cas
              ...
              <p>
                FireSight est développé comme un projet à mission : l&apos;objectif est l&apos;intérêt
          ...
          <>
            ...
            <section className="space-y-4 text-[15px] leading-relaxed">
              <p>
                FireSight is an independent information service that maps wildfire ignitions detected
              ...
              <p>
                ...
                earliness. FireSight adds social media monitoring of citizen reports, geolocated by
              ...
              <H2>Disclaimer</H2>
              <p className="rounded-[14px] p-4" style={{ background: "var(--ember-soft)", color: "#8C3A16" }}>
                FireSight is not an official alert service and never replaces emergency channels. If
              ...
              <H2>Who are we?</H2>
              <p>
                FireSight is built as a mission-driven project: the goal is public interest, not data
