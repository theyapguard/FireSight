...
export function EmergencyButton({ lang }: { lang: Lang }) {
  ...
  return (
    <a
      href={`tel:${number}`}
      aria-label={t.emergency(number)}
      className="flex h-[38px] shrink-0 items-center gap-1.5 rounded-full px-3.5 text-sm font-medium text-white transition-colors sm:gap-2 sm:px-[18px]"
    ...
    >
      ...
      {/* Le mot « Urgence ? » n'apparaît qu'à partir de sm : sur mobile, icône
          téléphone + numéro suffisent et libèrent la place pour « Contribuer ».
          Le libellé complet reste lu par les lecteurs d'écran (aria-label). */}
      <span className="hidden sm:inline">{t.emergencyWord}&nbsp;</span>
      {number}
    </a>
  );
}
