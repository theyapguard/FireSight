...
- `maquette/FireSight App Redesign v2.dc.html` — **la cible visuelle** de l'interface carte (ouvrable dans un navigateur). C'est la référence pixel : header clair, recherche, chips de période, flux « En direct », CTA central, fiche foyer.
...
2. **Header** : fond clair `rgba(251,249,244,.9)` + blur 12px, lockup = `assets/logo-symbole.svg` + « FireSight » en Fredoka 500 noir (jamais blanc sur clair). Bouton « Urgence ? 112 » en braise `--ember`, pill.
...
7. **CTA principal** : « M'alerter sur cette zone » en pill jaune flottant bas-centre, avec point pulsant ; état activé = charbon « Zone sous alerte — FireSight veille ✓ ».
...
9. **Micro-interactions** : `--ease-out`, 150–250ms ; hover = fond plus foncé ou lift léger ; press scale(0.98) ; animation signature `firesight-wave` sur tout indicateur « écoute en cours ». Focus ring jaune jamais supprimé.
