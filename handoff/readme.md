# FireSight — Charte graphique & Design System

FireSight (firesight.io) est une app mobile grand public de détection précoce des feux de forêt. Elle écoute les réseaux sociaux en temps réel, corrobore avec les données satellite et météo, et alerte les habitants avant les canaux officiels. Le nom vient du canari dans la mine : le symbole universel de l'alerte précoce — quand le canari chante, il est encore temps d'agir.
...
- **Casse** : phrases en sentence case. Le wordmark « FireSight » toujours en bas de casse. Pas de MAJUSCULES CRIANTES sauf overlines de section (12px, letterspacing 1.5px).
...
- **Exemples de copy** :
  ...
  - État calme : « Rien à signaler autour de toi. FireSight écoute. »
...
- **Animation** : rapide et optimiste. `--ease-out` (cubic-bezier(.2,.8,.3,1)), 150–250ms pour l'UI, 450ms pour les entrées de section. Signature : la pulsation d'onde `firesight-wave` (opacité 0.12→1, décalée par arc) sur le symbole et les indicateurs « écoute en cours ». Pas de bounce excessif, pas de parallax.
...
- **Lockup horizontal** : symbole + « FireSight » en Fredoka Medium, bas de casse, letterspacing -0.5px, espace = 20% de la hauteur du symbole.
...
- `ui_kits/website/` — homepage firesight.io de référence
- `SKILL.md` — pour utilisation dans Claude Code
- Exploration d'origine du logo : `FireSight Logo Concepts.dc.html` (3 concepts, reco = 1c)
